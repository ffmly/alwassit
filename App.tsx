...
const MOCK_USER: User = {
  ...
  // @ts-ignore - extending for compatibility if needed
...
const App = () => {
  ...
  // Using AppView enum where possible, but allowing string for new screens until types are updated
...
-500',
  ...
  const getActiveTab = (currentView: AppView | string) => {
      // Map AppView enums to string IDs expected by BottomNav
      ...
      if (typeof currentView === 'string') return mapping[currentView] || 'HOME';
...
Ryu, [24/11/2025 23:43]
nup={() => setView(AppView.SIGNUP_FORM)} 
                            />
                        )}
                        ...
                        {view === AppView.LOGIN && (
                            <LoginScreen 
                                ...
                                onLoginSuccess={() => { setIsAuthenticated(true); setView(AppView.HOME); }} 
                            />
                        )}
...
Ryu, [24/11/2025 23:43]
pView.LANDING, AppView.LOGIN, AppView.SIGNUP_FORM].includes(view as AppView) && (
