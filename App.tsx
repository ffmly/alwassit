...
const App = () => {
  ...
  // Common Header Logic
  const showHeader = [AppView.HOME, AppView.SEND_MONEY, AppView.LOANS, AppView.AI_ASSISTANT, AppView.BILLS, AppView.TOPUP, AppView.CREDIT_SCORE, AppView.MAP].includes(view);

  // Voice Assist for Seniors
  const showVoiceAssist = (user.age && user.age > 60) || (signupData.age && parseInt(signupData.age) > 60);
  ...
  const handleVoiceCommand = async () => {
    ...
    recognition.lang = 'ar-DZ'; // Try setting to Algerian Arabic (or generic Arabic)
    ...
    recognition.onresult = async (event: any) => {
      ...
      if (result.intent === 'SEND_MONEY') {
        setView(AppView.SEND_MONEY);
        // Ideally pre-fill amount if we had a global state manager or context
      } else if (result.intent === 'CHECK_BALANCE') {
        setView(AppView.HOME);
        // Could trigger a toast or voice response here
      } else if (result.intent === 'LOANS') {
        setView(AppView.LOANS);
      } else {
        alert(`Didn't understand: "${text}"`);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-transparent text-slate-900 font-sans max-w-md mx-auto relative overflow-hidden border-x border-white/20 perspective-2000 shadow-2xl">
          ...
          <VerificationScreen
            ...
            onComplete={() => {
              setUser({
                ...
                isVerified: true,
                age: parseInt(signupData.age) || 30,
      ...
      {/* Bottom Navigation */}
      {[AppView.HOME, AppView.SEND_MONEY, AppView.LOANS, AppView.AI_ASSISTANT, AppView.BILLS, AppView.TOPUP, AppView.MAP].includes(view) && (
        <BottomNav
          activeTab={AppView[view]}
          onTabChange={handleNav}
          showVoiceAssist={showVoiceAssist}
